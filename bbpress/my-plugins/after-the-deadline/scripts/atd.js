...
function check()
{
    jQuery(function()
    {
        ...
        {
            restoreTextArea();
        }else if(!jQuery('#post_content').val()){
            jAlert('Please enter some text in the post textbox to be checked!', 'Error!');
            restoreTextArea();
        }else
        {
            ...
            var disableClick = function() { return false; };
            jQuery('#checkLink').click(disableClick);
            jQuery('#post_content').replaceWith('<div class="input post_content" id="post_content">' + jQuery('#post_content').val() + '</div>');
            AtD.checkCrossAJAX('post_content',
            {
                ready: function(errorCount)
                {
                   jQuery('#checkLink').unbind('click', disableClick);
                },
...
var AtD = 
{
   ...
   check : function(container_id, callback_f)
   {
      ...
      jQuery.ajax({
         ...
         data : 'key=' + AtD.api_key + '&data=' + escape(text),
         format : 'raw', 
...
/*
 jQuery Alert Dialogs Plugin Section
*/
(function($) {
	$.alerts = {
		verticalOffset: -75,
